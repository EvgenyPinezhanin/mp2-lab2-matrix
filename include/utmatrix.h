...
{
	if (s < 0 || s > MAX_VECTOR_SIZE) throw logic_error("Invalid value for the Size of the vector");
	if (si < 0) throw logic_error("Invalid value for the StartIndex of the vector");
	Size = s;
	StartIndex = si;
...
{
	if (pos < StartIndex || pos >= StartIndex + Size) throw logic_error("Invalid index value");
